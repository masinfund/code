...
class Sales_dataTest{
	friend ostream &operator<<(ostream& , const Sales_dataTest&) ;
	friend istream& operator>>(istream& , Sales_dataTest&) ;
	friend bool operator==(const Sales_dataTest& , const Sales_dataTest &) ;

	friend ostream &print(ostream& , const Sales_dataTest &) ;
	friend istream &read(istream& , Sales_dataTest&) ;
public:

	//构造函数 
	Sales_dataTest() = default ; // 内联的
	Sales_dataTest(const string &s) : bookNo(s) { }
	// equal to Sales_data(cosnt std::string &s) : 
	//bookNo(s) , units_sold(0) , revenue(0) { } ,即其他执行隐式初始化

	Sales_dataTest(const string &s,unsigned n , double p) :
				bookNo(s),units_sold(n),revenue(p * n) { }
	Sales_dataTest(istream &) ;
	/*类内实现构造函数
	...
	*/
	...
	Sales_dataTest& operator+=(const Sales_dataTest&) ;
...
ostream &print(ostream &os, const Sales_dataTest& item) ;
istream &read(istream&is , Sales_dataTest&item) ; // 读入会修改数据，no const . 


Sales_dataTest operator+(const Sales_dataTest & , const Sales_dataTest &) ;
ostream &operator<<(ostream& , const Sales_dataTest &) ;
istream &operator>>(istream& , Sales_dataTest &) ;


inline 
bool operator==(const Sales_dataTest& lhs, const Sales_dataTest &rhs) 
{
