...
//构造函数
Sales_dataTest::Sales_dataTest(istream &is)
{
...
istream &read(istream &is , Sales_dataTest &item) 
{
	double price = 0 ;
...
ostream &operator<<(ostream &os , const Sales_dataTest &item)
{
	return print(os,item) ;
}

istream &operator>>(istream &is , Sales_dataTest &item)
{
	return read(is,item) ;
 // cout << a << b ;  ==     operator << (operator << (cout ,a) ,b) ;
}

Sales_dataTest& Sales_dataTest::operator+=(const Sales_dataTest &rhs){
	units_sold += rhs.units_sold ;
	revenue += rhs.revenue ;
	return *this ;
}

Sales_dataTest operator+(const Sales_dataTest &lhs , const Sales_dataTest &rhs)
{
	Sales_dataTest sum = lhs ;
	sum += rhs ;
	return sum ;
}
